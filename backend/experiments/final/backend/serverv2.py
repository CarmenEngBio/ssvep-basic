...
async def run_blocks(ws, source):
    ...
    # Arranca grabación de ESTA celda (fichero con el nombre de la celda)
    source.get_new_samples()                     # vacía backlog previo
    ...
    while elapsed_trial < TRIAL_SEC and recorder.is_recording:
        ...
        raw_eeg, raw_ts = await source.get_window()   # 4s de EEG
    ...
    # A los 40 s: escribe el tramo y PARA de escribir
    ...
    for cell_id in sorted(CELLS.keys()):
        ...
        while elapsed_trial < TRIAL_SEC and recorder.is_recording:
            ...
            if raw_eeg.shape[1] > 0 and recorder.is_recording:                
                ...
                print(f"[Result] {emoji} {label}: Corr={result['corr']:.4f} — {status}")
                print(f"[Time elapsed] {elapsed_block:.4f} (s)")
                print(f"\n[SUMMARY] Accuracy: {num_corrects}/{num_blocks} ({acc_trial:.1f}%)")
                ...
                await ws.send(json.dumps({
                    "type": "session_ended", #block_result
        ...
        # Grabamos datos de trial completo 40 s en fichero
        new_eeg, new_ts = source.get_new_samples()
        if recorder.is_recording:
            recorder.write_chunk(new_eeg, new_ts)
